...
import Search from "./Search";
  ...
  const UsersFilter = () => {
    ...
    const [usersToShow, setUsersToShow] = useState(8); // Start with 8 items displayed
    ...
    const filteredUsers = users.filter((user) => {
      ...
      const matchesCategory = searchParams.category
        ? user.secteur.toLowerCase().includes(searchParams.category.toLowerCase())
        : true;
      const matchesActivity = searchParams.activity
        ? user.activité.toLowerCase().includes(searchParams.activity.toLowerCase())
        : true;
  ...
  useEffect(() => {
    ...
    const handleSeeMore = () => {
      setUsersToShow((prev) => prev + 8); // Show 8 more users
    };
